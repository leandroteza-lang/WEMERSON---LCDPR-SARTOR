...
//     WITH CHECK: (id = ( SELECT profiles.company_id    FROM profiles   WHERE (profiles.id = auth.uid())))
// Table: lcdpr_files
//   Policy "Users can access their files" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (producer_id IN ( SELECT producers.id    FROM producers   WHERE (producers.company_id = ( SELECT profiles.company_id            FROM profiles           WHERE (profiles.id = auth.uid())))))
//     WITH CHECK: (producer_id IN ( SELECT producers.id    FROM producers   WHERE (producers.company_id = ( SELECT profiles.company_id            FROM profiles           WHERE (profiles.id = auth.uid())))))
// Table: producers
